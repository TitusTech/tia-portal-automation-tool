...
def create_device_network_service(imports: Imports, device_data: dict[str, Any], device: Siemens.Engineering.HW.Device) -> list[Siemens.Engineering.HW.Features.NetworkInterface]:
    ...
    for deviceItem in device_items:
        logging.debug(f"Accessing a NetworkInterface at DeviceItem {deviceItem.Name}")

        network_service: Siemens.Engineering.HW.Features.NetworkInterface = SE.IEngineeringServiceProvider(deviceItem).GetService[SE.HW.Features.NetworkInterface]()
        if not network_service:

            logging.debug(f"No NetworkInterface found for DeviceItem {deviceItem.Name}")

        logging.debug(f"Found NetworkInterface for DeviceItem {deviceItem.Name}")
