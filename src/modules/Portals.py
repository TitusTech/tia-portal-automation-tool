...
def connect(imports: Imports, config: dict[Any, Any], settings: dict[str, Any]) -> Siemens.Engineering.TiaPortal:
    ...
    connection_method: dict = settings.get(
        'connection_method', {'mode': 'new'})
    ...
    if connection_method.get('mode') == 'attach':
        ...
        if process_id and isinstance(process_id, int):
            ...
            logger.info(f"Attached TIA Portal Openness ({process.Id}) {
                        process.Mode} at {process.AcquisitionTime}")
    ...
    logger.info(f"Started TIA Portal Openness ({process.Id}) {
                process.Mode} at {process.AcquisitionTime}")
